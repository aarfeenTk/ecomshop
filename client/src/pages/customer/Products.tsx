...
const Products: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (category && category.length > 0) {
      filtered = filtered.filter((product: Product) => category.includes(product.category));
    }

    setFilteredProducts(filtered);
  }, [products, searchTerm, category]);

  const categories = [...new Set(products.map((product: Product) => product.category).filter(Boolean)) as string[]];
